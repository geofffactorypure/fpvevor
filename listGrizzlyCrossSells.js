...
const STORE_ID = 1
// argv[2]: limit (default 10)
// argv[3]: concurrency (default 3)
const LIMIT = parseInt(process.argv[2]) || 10
const CONCURRENCY = parseInt(process.argv[3]) || 3
const RETRY_MODE = process.argv.includes('--retry')
const RETRY_FILE = new URL('./grizzly_cross_sell_lister_retry.json', import.meta.url)
const SHOPIFY_API_VERSION = '2026-01'

// 10% margin is the minimum target for cross-sell products.
// If the natural price (MAP/MSRP) is below this, we raise the price and flag it.
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if (attempt < retries) {
                ...
                console.warn(`[fetch] Network error for ${url} (attempt ${attempt}/${retries}), retrying in ${wait / 1000}s: ${err.message}`)
        ...
        if (pageRes.status === 429 || pageRes.status === 503) {
            ...
            console.warn(`[fetch] Rate limited (${pageRes.status}) for ${url} (attempt ${attempt}/${retries}), waiting ${wait / 1000}s...`)
            await new Promise((r) => setTimeout(r, wait))
            continue
        }
        ...
        if (!productMatch) {
            if (attempt < retries) {
                ...
                console.warn(`[fetch] Got 200 but no window.product for ${url} (attempt ${attempt}/${retries}), waiting ${wait / 1000}s...`)
                await new Promise((r) => setTimeout(r, wait))
                continue
            }
            return null
...
async function createShopifyProduct({ scrapedResult, product_type, vendor, sku, storeInfo, pool, priceRaised = false }) {
    const media =
    ...
    const product = {
        ...
        tags: ['new', 'Grizzly Cross Sell Lister', ...(priceRaised ? ['grizzly_special_pricing'] : []), ...tags.filter(Boolean)],
        metafields: [
            {
                namespace: 'custom',
                key: 'warranty',
...
async function main() {
    ...
    try {
        // 1. Load griz_price_list.csv into a map keyed by SKU for pricing lookups
        console.log('Loading griz_price_list.csv...')
        const priceCsvPath = new URL('./griz_price_list.csv', import.meta.url)
        const priceCsvContent = fs.readFileSync(priceCsvPath, 'utf-8')
        const priceRows = parse(priceCsvContent, {
            columns: true,
        ...
        for (const row of priceRows) {
            ...
            if (sku) priceMap.set(sku, row)
        }
        console.log(`Loaded ${priceMap.size} SKUs from griz_price_list.csv`)

        // 2. Load SKU type mapping if available
        ...
        // 3. Load grizzly_unlisted_cross_sells.csv
        console.log('Loading grizzly_unlisted_cross_sells.csv...')
        const crossSellsCsvPath = new URL('./grizzly_unlisted_cross_sells.csv', import.meta.url)
        const crossSellsContent = fs.readFileSync(crossSellsCsvPath, 'utf-8')
        const crossSellRows = parse(crossSellsContent, {
            columns: true,
            skip_empty_lines: true,
            relax_column_count: true,
            relax_quotes: true,
        ...
        // 4. Filter to unlisted only and build enriched rows with pricing
        const allRows = []
        for (const csRow of crossSellRows) {
            const sku = csRow['accessory_sku']?.trim()
            if (!sku) continue
            if ((csRow['listed'] || '').toLowerCase() === 'yes') continue

            const priceRow = priceMap.get(sku)
            if (!priceRow) {
                console.warn(`[${sku}] No pricing data in griz_price_list.csv — skipping`)
                continue
            }

            const csvBrand = priceRow['Brand']?.trim()
            const vendor = normalizeBrand(csvBrand)

            // Only list brands we carry
            const ALLOWED_VENDORS = new Set(['Grizzly', 'Shop Fox', 'South Bend'])
            if (!ALLOWED_VENDORS.has(vendor)) {
                console.warn(`[${sku}] Vendor "${vendor}" not in allowed list — skipping`)
                continue
            }

            if ((priceRow['Discontinued'] || '').trim().toLowerCase() === 'true') {
                console.warn(`[${sku}] Discontinued — skipping`)
                continue
            }

            const msrp = parseFloat((priceRow['MSRP'] || '').replace(/[^0-9.]/g, '')) || 0
            const dealerMap = parseFloat((priceRow['Dealer MAP'] || '').replace(/[^0-9.]/g, '')) || 0
            const dealerPrice = parseFloat((priceRow['Dealer Price'] || '').replace(/[^0-9.]/g, '')) || 0
            const rawShipping = (priceRow['Dealer Shipping Cost'] || '').trim()
            const shippingCost =
            ...
            if (dealerPrice <= 0) {
                console.warn(`[${sku}] No dealer price found — skipping`)
                continue
            }

            // Natural price from MAP or MSRP (same rules as the main lister)
            ...
            let naturalPrice
            if (dealerMap > 0) {
                naturalPrice = dealerMap
            } else {
                naturalPrice = msrpFallback
            }

            // Minimum price to achieve the 10% target margin
            // margin = (price - cost - shipping) / price >= TARGET_MARGIN
            // => price >= (cost + shipping) / (1 - TARGET_MARGIN)
            const minPriceForTarget = Math.ceil((dealerPrice + shippingCost) / (1 - TARGET_MARGIN))

            const price = Math.max(naturalPrice, minPriceForTarget)
            const priceRaised = price > naturalPrice

            if (priceRaised) {
                const origMargin = naturalPrice > 0
                    ? ((naturalPrice - dealerPrice - shippingCost) / naturalPrice * 100).toFixed(1)
                    : '0.0'
                const newMargin = ((price - dealerPrice - shippingCost) / price * 100).toFixed(1)
                console.log(
                    `[${sku}] Price raised $${naturalPrice} → $${price} (margin ${origMargin}% → ${newMargin}%) — will set special_pricing`
                )
            }
            ...
            allRows.push({
                ...
                vendor,
                csvBrand,
                upc: priceRow['UPC']?.trim() || '',
                description: priceRow['Description']?.trim() || '',
                price,
                naturalPrice,
                priceRaised,
                cost: dealerPrice,
                ...
                shipType: priceRow['Ship Type']?.trim() || '',
                productType,
                itemStatus: priceRow['Item Status']?.trim(),
                margin,
            })
        }

        console.log(`Found ${allRows.length} unlisted cross-sell product(s) with pricing data`)

        if (allRows.length === 0) {
            console.log('No products to list. Exiting.')
            return
        }
        ...
        // 6. Filter out already-listed SKUs from DB
        console.log('Loading already-listed SKUs from database...')
        const vendorsPresent = [...new Set(allRows.map((r) => r.vendor))]
        const placeholders = vendorsPresent.map(() => '?').join(', ')
        ...
        console.log(
            `${toProcess.length} unlisted cross-sell product(s) to process (${listedSkuSet.size} excluded as already listed)`
        )
        ...
        // 8. Get publications and primary location
        ...
        if (RETRY_MODE) {
            ...
            console.log(`Retrying ${filteredToProcess.length} previously failed SKU(s) from ${retrySkus.length} in retry file\n`)
        }

        const toList = filteredToProcess.slice(0, LIMIT)
        console.log(`\nListing ${toList.length} cross-sell product(s) with concurrency ${CONCURRENCY}...\n`)
        ...
        async function listOneProduct(row) {
            ...
            if (variantGid) {
                ...
                console.log(
                    `[${sku}] ✓ Variant updated (price: $${price}, cost: $${cost}, sku: ${sku}${priceRaised ? ', note: special_pricing' : ''})`
                )
            ...
            await createListingEvent(pool, {
                ...
                event_data: JSON.stringify({
                    source: 'GRIZZLY_CROSS_SELL_LISTER_SCRIPT',
                    sku,
                    product_type: resolvedProductType,
                    vendor,
                    price: String(price),
                    unit_cost: String(cost),
                    price_raised: priceRaised,
                }),
                store_id: STORE_ID,
            }).catch((err) => {
        ...
        if (failedSkus.length > 0) {
            ...
            console.log(`\n  Saved ${failedSkus.length} failed SKU(s) to grizzly_cross_sell_lister_retry.json — run with --retry to retry them`)
        } else if (RETRY_MODE) {
